...
from common.utils import LearnRates, ParamSamplerResults, RhoElem, TrajectoryResults
from simulation.trajectory_sampler import ParameterSampler, pgpe_sampling_worker
...
class PGPE:
    ...
    def __init__(
        ...
        if self.lr_strategy == "adam":
            ...
            self.rho_adam[RhoElem.MEAN] = Adam(alpha=self.lr)
            self.rho_adam[RhoElem.STD] = Adam(alpha=self.lr)
    ...
    def learn(self) -> None:
        ...
        for i in tqdm(range(self.ite)):
            ...
            best_theta_batch_index = int(np.argmax(performance_res))
    ...
    def update_rho(self) -> None:
        ...
        if not self.natural:
            ...
            log_nu_stds = (((self.thetas - means) ** 2) - (stds ** 2)) / (stds ** 3)
        else:
            log_nu_means = self.thetas - means
            log_nu_stds = (((self.thetas - means) ** 2) - (stds ** 2)) / (2 * stds)
        ...
        if self.lr_strategy == "constant":
            ...
            if self.learn_std:
                new_std = self.rho[RhoElem.STD, :] + self.lr * np.mean(grad_stds, axis=0)
                self.rho[RhoElem.STD, :] = np.clip(new_std, self.std_min, np.inf)
        elif self.lr_strategy == "adam":
            adaptive_lr_m = self.rho_adam[RhoElem.MEAN].next(np.mean(grad_means, axis=0))
            adaptive_lr_m = np.array(adaptive_lr_m)
            self.rho[RhoElem.MEAN, :] = self.rho[RhoElem.MEAN, :] + adaptive_lr_m
            if self.learn_std:
                adaptive_lr_s = self.rho_adam[RhoElem.STD].next(np.mean(grad_stds, axis=0))
                adaptive_lr_s = np.array(adaptive_lr_s)
                new_std = self.rho[RhoElem.STD, :] + adaptive_lr_s
                self.rho[RhoElem.STD, :] = np.clip(new_std, self.std_min, np.inf)
        else:
            raise NotImplementedError("[PGPE] Not implemented!")
    ...
    def update_best_theta(self, current_perf: float, params: np.array, *args, **kwargs) -> None:
        ...
        if current_perf > self.best_performance_theta:
            self.best_theta = copy.deepcopy(params)
