...
class Continuous_MountainCarSimmEnv_v5(gym.Env):
    """
    ## Description
    ...
    * v0: Initial versions release
    ...
    def step(self, action: np.ndarray):
        ...
        if ( -math.pi/12 < position < math.pi/12) or (-11*math.pi/12 <position< -3*math.pi/4) or (3*math.pi/4<position<11*math.pi/12):
            ...
            # attrito versione non naive, forse dovrei moltiplicare per 0.0025
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        if options is None:
            ...
            start = options["start"]
            v = self.np_random.uniform(low = -0.02, high = 0.02)
            self.state = np.array([start, v])
    ...
    def _height(self, xs):
        ...
        y = np.where(
            ...
            np.where(
                ...
                np.where(
                    x < 3*math.pi/4, np.sin(3*(x + math.pi/12)) * 0.45 + 0.55,
                    np.where(
                        x < 11*math.pi/12, 1,
                        np.where(
                            x < 13*math.pi/12, np.sin(3*(x - math.pi/12)) * 0.45 + 0.55,
