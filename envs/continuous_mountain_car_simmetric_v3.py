...
class Continuous_MountainCarSimmEnv_v3(gym.Env):
    """
    ## Description
    ...
    * v0: Initial versions release
    ...
    def step(self, action: np.ndarray):
        ...
        if position < math.pi/12 and position > -math.pi/12:
            #attrito versione naive
            velocity+= (force + force_friction ) * self.power
        else:
            if position >= math.pi / 12:
                displacement = math.pi /12
            elif position <= -math.pi / 12:
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        if options is None:
            ...
            low, high = np.array([-2.0, -0.02] ), np.array([2.0, 0.02])
        else :
            #low, high = utils.maybe_parse_reset_bounds(options, -0.6, -0.4)
            low, high = utils.maybe_parse_reset_bounds(options, -2.00, 2.00)
    ...
    def _height(self, xs):
        ys = np.where(xs <= -math.pi/12,
            np.sin(3 * (xs + math.pi/4)) * 0.45 + 0.55,
            np.where(xs >= math.pi/12,
                    np.sin(3 * (xs + math.pi/12)) * 0.45 + 0.55,
                    1)
        )
        return ys
    ...
    def render(self):
        ...
        for c in [(l, b), (l, t), (r, t), (r, b)]:
            if pos <= -math.pi/12:
                c = pygame.math.Vector2(c).rotate_rad(math.cos(3 * (pos + math.pi/4)))
            elif pos >= math.pi/12:
                c = pygame.math.Vector2(c).rotate_rad(math.cos(3 * (pos + math.pi/12)))
            else :
